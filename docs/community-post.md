...
7. **Live models.dev lookup** - optional, opt-in; fetches current sizes and caches them (default off, for privacy)
8. **Built-in table** - seeded from [models.dev](https://models.dev), covers the popular OpenAI / Anthropic / Gemini / Llama / DeepSeek / Grok / Mistral / Qwen / Kimi / GLM / MiniMax / Cohere families
...
The backend did not advertise its running window. Behind llama-swap or a llama.cpp router the connection probe needs the model listed through Open WebUI's connection to it (a connection restricted to hand-typed model ids has no listing to detect it from) and bearer or no auth. Otherwise update llama.cpp (its `/v1/models` lists `meta.n_ctx` since May 2026) and refresh Open WebUI's model list, set `capabilities.context` in llama-swap, set the `llamacpp_url` / `llama_swap_url` valve, or add a `context_size_map` entry. `model.backend_context` in the debug payload shows the running (`n_ctx`) and trained (`n_ctx_train`) windows the listing carries.
